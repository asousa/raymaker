...
dump_model = False
run_rays   = True
vec_ind = 3     # Which set of default params to use for the gcpm model
# Kpvec = [0, 2, 4, 6, 8]

ray_out_dir = '/shared/users/asousa/WIPP/rays/2d/dayside/gcpm_kp6'
...
if (dump_model):
    ...
    if (rank < len(chunks)):
        ...
        cmd = '%s '%os.path.join(ray_bin_dir, 'dumpmodel') +\
                ...
                '--tsyganenko_W6=%g '%(W[5]) +\
                '--gcpm_kp=%g '%(Kp) +\
                '--MLT=%g --fixed_MLT=%g'%(MLT, fixed_MLT)
